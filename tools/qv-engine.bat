...
REM ============================================================================
REM  OCHA QuickVid - engine start logic. Lives in tools\ ON PURPOSE: this file
REM  IS reached by the self-update mirror, so bugs in here can be fixed
REM  remotely. The thin launcher ("OCHA QuickVid.bat" in the app root) is the
REM  part the mirror must exclude, so it stays as small as possible.
REM ============================================================================
REM ============================================================================
...
REM /XF excludes the two files that CANNOT be replaced while they run: the thin
REM launcher (never updated by design) and THIS script. The new copy of this
REM script is staged as tools\qv-engine.new.bat instead, and the launcher swaps
REM it in on the next start - that is what makes a broken engine script fixable
REM remotely (before the split, it wasn't; see docs/decisions.md 2026-07-27).
robocopy "%UTMP%\quickvid_BDU-main" "%CD%" /MIR /IS /XD ".venv" ".git" /XF "OCHA QuickVid.bat" "qv-engine.bat" /NFL /NDL /NJH /NJS /NC /NS /NP >nul
copy /y "%UTMP%\quickvid_BDU-main\tools\qv-engine.bat" "%CD%\tools\qv-engine.new.bat" >nul 2>&1
